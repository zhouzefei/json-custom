...
export class Node {
    ...
    changeType(newType) {
        ...
        if ((newType === 'string' || newType === 'auto') &&
            (oldType === 'string' || oldType === 'auto') && false) {
    ...
    getDom() {
        ...
        let operas = this.showContextMenu();
        ...
        if (this.editor.options.mode === 'tree') { // note: we take here the global setting
            ...
            menu.type = 'button'
            dom.menu = menu
            menu.className = `jsoneditor-button ${textObj.className}`
            menu.title = translate('actionsMenu')
            tdMenu.appendChild(dom.menu)
            dom.tr.appendChild(tdMenu)
        }
        // 增加操作按钮
        operas = operas.filter((opera) => {
            return ["jsoneditor-remove", "jsoneditor-duplicate"].indexOf(opera.className) > -1
        })
        if (operas && operas.length > 0) {
            operas.map((opera) => {
                const td = document.createElement('td')
                const div = document.createElement("div")
                div.className = "jsoneditor-contextmenu"
                const button = document.createElement('button')
                button.type = "button"
                button.className = opera.className
                button.innerHTML = '<div class="jsoneditor-icon" style="cursor:pointer"></div>'
                if (opera.title) {
                    button.title = opera.title
                }
                if (opera.click) {
